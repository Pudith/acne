...
# imageFiles = [join(originalFile_Dir, f) for f in listdir(originalFile_Dir) if isfile(join(originalFile_Dir, f)) and '.md' not in f and '.DS_Store' not in f]
imageFiles = [join(originalFile_Dir, 'input_img.jpg')]
...
for imagefile in imageFiles[start_index:end_index]:
    ...
    if face_detected: # if face is detected by landmark model
        ...
        if verb:
            ...
            plt.imshow(img_tmp)
            all_file_name = join(croppedFaces_Dir, imageName+"_all.jpg")
#             img_tmp = cv2.cvtColor(img_tmp, cv2.COLOR_BGR2RGB)
            imageio.imwrite(all_file_name, img_tmp)
    ...
    if not face_detected:
        ...
        if max_area >= min_eye_area: # if area of maximal eye is greater than the eye area threshold, take it as a real eye
            ...
            if verb:
                ...
                all_file_name = join(croppedFaces_Dir, imageName+"_all.jpg")
                imageio.imwrite(all_file_name, image)
